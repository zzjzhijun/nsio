...
private:
    struct awaiter_base
    {
        ...
        explicit awaiter_base(std::coroutine_handle<promise_type> current) noexcept : handle(current)
        {
        }
...
public:
    ...
    explicit task(std::coroutine_handle<promise_type> current) noexcept : handle(current)
    {
    }
...
auto make_task(Awaiter awaiter)
